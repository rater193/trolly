...
export class State {
  /**
   * @param {object} opts
   * @param {import("./sync.js").SyncClient} opts.sync  shared-data transport
   * @param {object} opts.user  the logged-in user ({ id, username })
   */
  constructor({ sync, user } = {}) {
    this.bus = new EventBus();
    this.storage = new Storage(); // local-only: theme, sidebar, current selection
    this.sync = sync;
    this.user = user || null;

    // The shared workspace tree is delivered by the server, not loaded here.
    this.workspaces = [];

    const ui = this.storage.loadUi();
    this.currentBoardId = ui.currentBoardId || null;
    this.currentWorkspaceId = ui.currentWorkspaceId || null;
    this.sidebarCollapsed = !!ui.sidebarCollapsed;
    this.theme = this.storage.loadTheme();
    this.filter = { labelIds: new Set(), due: null, search: "" };
  }

  /** Seed the tree from the server's initial payload (no re-render needed). */
  hydrate(workspacesJSON) {
    this.workspaces = (workspacesJSON || []).map(Workspace.fromJSON);
    this._reconcileSelection();
  }

  /**
   * Replace the tree with a server update from another client and re-render.
   * Keeps the user's local view (current board/workspace) when it still exists.
   */
  applyRemote(workspacesJSON) {
    this.workspaces = (workspacesJSON || []).map(Workspace.fromJSON);
    this._reconcileSelection();
  ...
  _reconcileSelection() {
    if (this.currentBoardId && !this.findBoard(this.currentBoardId)) this.currentBoardId = null;
    if (this.currentWorkspaceId && !this.findWorkspace(this.currentWorkspaceId)) this.currentWorkspaceId = null;
    if (!this.currentWorkspaceId) this.currentWorkspaceId = this.workspaces[0]?.id || null;
  }

  /* --------------------------- Persistence --------------------------- */
  persist() {
    // Shared data goes to the server; view/UI preferences stay local per-browser.
    this.sync?.save(this.workspaces);
    this.storage.saveUi({
  ...
  addComment(boardId, listId, cardId, text) {
    const card = this.findCard(boardId, listId, cardId);
    if (!card) return;
    card.comments.push({ id: uid("cm"), text, ts: Date.now(), author: this.user?.username || null });
    this.persist();
    this.bus.emit("card:updated", { boardId, listId, card });
  }
